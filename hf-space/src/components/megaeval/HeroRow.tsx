...
const VERIFIED_CHIP = (
  <Chip
    ...
    sx={{ height: 16, fontSize: 9, fontWeight: 700, bgcolor: '#12303f', color: '#81d4fa', ml: 0.5 }}
  />
);
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
      <Box sx={{ display: 'flex', gap: 1.5, flexWrap: 'wrap' }}>
        <MetricCard label="Objects evaluated" value={fmtCompact(h.n)} sub="exact (cube)" />
        <MetricCard
          label="Containers"
          value={fmtCompact(h.containers)}
          sub={h.containers_hll_estimate !== undefined ? `HLL est ${fmtCompact(h.containers_hll_estimate)} (approx)` : 'exact'}
        />
        <MetricCard
          label="Reviewed"
          ...
          accent="#42a5f5"
        />
        <MetricCard label="Verified" value={fmtCompact(verified)} accent="#66bb6a" sub="human-verified" />
      </Box>
      <Box sx={{ display: 'flex', gap: 1.5, flexWrap: 'wrap' }}>
        <MetricCard
          label="Precision"
        ...
        />
        <MetricCard label="F1" value={fmtPct(h.f1)} />
        <MetricCard label="Mean IoU" value={fmtNum(h.mean_iou, 3)} />
        <MetricCard label="Anomaly rate" value={fmtPct(h.anomaly_rate)} accent="#ffa726" />
        <MetricCard label="Safety recall" value={fmtPct(h.safety_recall)} accent="#ef5350" />
      </Box>
    </Box>
  );
}
